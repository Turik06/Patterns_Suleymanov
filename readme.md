...
|---|---|---|
| **Единица измерения** (`Unit`) | Простая сущность. Мера веса или объёма: граммы, литры, штуки. | → `Ингредиент`, → `Группа` |
| **Деньги** (`Money`) | Простая сущность. Сумма и валюта для цен и расчётов. | → `Блюдо / Номенклатура блюд`, → `Контракт` |
| **Ингредиент** (`Ingredient`) | Простая сущность. Продукт с указанием количества и меры. | → `Единица измерения`, → `Блюдо / Номенклатура блюд`, → `Технологическая карта`, → `Склад` |
| **Адрес / Адрес доставки** (`Address`) | Простая сущность. Местоположение ресторана или клиента для доставки. | → `Ресторан`, → `Заказ` |
| **Сотрудник** (`Employee`) | Простая сущность. Работник компании. | → `График работы`, → `Контракт`, → `Ресторан` |
| **Контрагент** (`Contractor`) | Простая сущность. Поставщик продуктов или партнёр компании. | → `Группа`, → `Контракт` |
| **Группа** (`Group`) | Простая сущность. Категория для объединения похожих объектов. | → `Единица измерения`, → `Контрагент`, → `Группа`|
| **Блюдо / Номенклатура блюд** (`Dish`) | Комплексная сущность. Позиция меню с ценой и списком продуктов для приготовления. | → `Ингредиент`, → `Деньги`, → `Технологическая карта`, → `Ресторан` |
| **Технологическая карта** (`Recipe`) | Комплексная сущность. Рецепт с точным весом ингредиентов и шагами готовки. | → `Ингредиент`, → `Блюдо / Номенклатура блюд`, → `Технологическая карта` |
| **Склад** (`Storage`) | Комплексная сущность. Место хранения запасов продуктов и заготовок. | → `Ингредиент`, → `Ресторан`, → `Заказ` |
| **Ресторан** (`Restaurant`) | Комплексная сущность. Точка сети со своим складом, меню и персоналом. | → `Адрес`, → `Склад`, → `Блюдо / Номенклатура блюд`, → `Сотрудник`, → `Заказ` |
| **Контракт** (`Contract`) | Комплексная сущность. Договор с поставщиком на закупку продуктов. | → `Контрагент`, → `Сотрудник`, → `Деньги` |
| **График работы** (`WorkSchedule`) | Комплексная сущность. Расписание рабочих смен сотрудников. | → `Сотрудник` |
| **Заказ** (`Order`) | Комплексная сущность. Заказ клиента со списком блюд и адресом доставки. | → `Ресторан`, → `Склад`, → `Блюдо / Номенклатура блюд`, → `Адрес / Адрес доставки` |
